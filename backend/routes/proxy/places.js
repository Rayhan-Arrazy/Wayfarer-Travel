...
router.get("/nearby", async (req, res) => {
  try {
    ...
    const rad = parseInt(req.query.radius) || 1000; // Default to 1km if not specified
  ...
  try {
    ...
    try {
      const data = await callWithFallback(
        ...
        () =>
          axiosGet(
            `https://router.project-osrm.org/route/v1/${routeProfile === "driving-car" ? "driving" : "foot"}/${startLng},${startLat};${endLng},${endLat}`,
            {
  ...
  try {
    ...
    if (!data) {
      data = await callWithFallback(
        () =>
          axiosGet(
            `https://en.wikipedia.org/api/rest_v1/page/summary/${encodeURIComponent(req.params.place)}`,
          ),
        () =>
