...
class _ToolsTabScreenState extends State<ToolsTabScreen> {
  ...
  void _calculateConversion(bool fromLeft) {
    ...
    if (fromLeft) {
      ...
      _rightCurrencyController.text = result.toString(); // Unlimited digits
    } else {
      final text = _rightCurrencyController.text.replaceAll(',', '');
      final amount = double.tryParse(text) ?? 0.0;
      final result = cp.convert(amount, _rightCurrency, _leftCurrency);
      _leftCurrencyController.text = result.toString(); // Unlimited digits
    }
    _isProcessing = false;
  }
  ...
  Future<void> _translateText() async {
    if (_translationInputController.text.isEmpty) return;
  ...
  Future<void> _speak() async {
    try {
      ...
      await _flutterTts.setLanguage(_languages[_selectedTargetLang]!);
    ...
    return Column(
      ...
      children: [
        ...
        Container(
          ...
          padding: const EdgeInsets.all(24),
          decoration: BoxDecoration(
            color: const Color(0xFFA6BCDB).withOpacity(0.3),
